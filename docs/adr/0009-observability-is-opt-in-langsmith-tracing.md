...
- **The tracing gate.** Tracing arms only when `LANGSMITH_TRACING` is truthy *and* `LANGCHAIN_API_KEY` is set; otherwise no tracer is constructed and every scope is a no-op. A test asserts the default: disabled. Env-only, never committed (public repo).
