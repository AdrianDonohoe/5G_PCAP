...
print to stdout as a JSON array; progress and memory notes go to stderr, so
stdout stays machine-readable. Zero Incidents is a successful empty result
(exit 0); exit 1 means the invocation itself failed.

GROQ_API_KEY must be set for any live invocation (ADR-0002: no local-model
fallback); a missing key surfaces as an honest error before any search
starts. Mid-search LLM failures degrade inside the tree by design.
"""
